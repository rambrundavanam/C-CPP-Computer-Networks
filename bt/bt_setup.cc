...
void __parse_peer(peer_t * peer, char * peer_st, int type){
  char * parse_str;
  char * word;
  unsigned short port;
  char * ip;
  ...
  init_peer(peer, id, ip, port, type);
...
void parse_args(bt_args_t * bt_args, int argc,  char * argv[]){
  ...
  while ((ch = getopt(argc, argv, "hp:s:l:vI:b:")) != -1) {
    ...
    case 'p': //peer
      ...
      //parse peers
      __parse_peer(bt_args->peers[n_peers], optarg, 0);
      n_peers++;
      break;
    case 'I':
    ...
    case 'b':
      ...
      __parse_peer(bt_args->peers[n_peers], optarg, 1);
      n_peers++;
      cout<<"NUMBER OF PEERS IS "<<n_peers<<endl;
      break;
    default:
