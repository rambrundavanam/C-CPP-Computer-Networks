...
typedef struct peer{
  ...
  int choked; //peer choked? unchoked = 1
  int interested; //peer interested? if yes, = 1
  int type; // 0 is leecher/client, 1 is seeder/server
}peer_t;
...
typedef struct {
  ...
  int num_blocks;//number of blocks for each file
} bt_info_t;
...
typedef struct {
  ...
  vector<int> sockets; //Array of possible sockets
...
string read_blocks(bt_info_t input);
...
string generate_request_message(bt_args_t *bt_args, bt_msg_t *my_message, int index, int g);
...
string parse_bitfield(string raw, int length);
...
int inital_handshake();

vector<int> initialize_client_sockets(bt_args_t bt_args);

vector<int> initialize_server_sockets(bt_args_t bt_args);

void peer_write(int socket, string message);

string peer_read(int my_socket, int length);

string server_read(bt_args_t, int i, int length);

void server_write(bt_args_t, int i, string message);

string client_read(bt_args_t, int i, int length);

void client_write(bt_args_t, int i, string message);
